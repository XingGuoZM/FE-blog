...
```
### 快速排序 
取数组中间项，以此为基准，将数组其他项和它相比较，大的放右边，小的放左边，然后左边和右边分别递归。
代码实现
```
 /**
 * 快速排序
...
```
### 直接插入排序  
从数组第二项开始，依次取出数组项插入到由数组第一项组成的数组，小的插前面，大的插后面，直到原数组项全部取出并插入完成排序。
代码实现
```
/**
 * 直接插入排序  
...
```
### 希尔排序 
和直接插入排序类似，定义间距（步长）取数组项进行比较，也就是相隔几项取出项来插入。
代码实现
```
/**
 * 希尔排序
...
### 堆排序 
构建二叉树，使用递归，将最大项置于根，然后将根和树最右端的子叶交换，剔除。
代码实现 
```
/**
 * 堆排序
...
```
### 归并排序  
将数组不断进行二分，递归比较大小，小的推进左队列，大的推进右队列，最后合并左后数组及中间项.
代码实现
...
```
### 计数排序  
新创建一个数组，将数组项的值作为新数组的索引（下标），新数组的值为待排序数组出现的次数，然后通过新数组的值取下标。
代码实现
```
/**
 * 计数排序
...
```
### 桶排序  
划分数值范围，将数组项值的大小填到对应的范围中，在对应范围内有数据项进行排序，最后进行有数组项的范围进行拼接。
代码实现
```
/**
 * 桶排序
...
```
### 基数排序 
对数组项从个位，十位.....到数组最大项的位数进行排序，然后取出数组项。
代码实现 
```
/**
 * 基数排序
